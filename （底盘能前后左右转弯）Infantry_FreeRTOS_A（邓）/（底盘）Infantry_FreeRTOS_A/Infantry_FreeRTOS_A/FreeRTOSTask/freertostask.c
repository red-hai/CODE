...
{	
	...
	xTaskCreate(DbusParseTask,       "dbustask"           ,1027,NULL,2,&DbusParseTask_Handler);	
...
//     #endif	
    ...
    for(;;)
    {
		    if(1) //遥控初始化成功
			{
				 ...
				 {
					 RemotData.err[0]=RemotData.rc.ch0-1024;//遥控通道0校准
					 start_cnt = 0;
					 StrartingMusic();
			  ...
			  case RC_SW_DOWN:
				  {
					  if(RemotData.mousekey.v == KEY_PRESSED_OFFSET_W)
							control.CMControl.forwardOrbackward =   FORWARESPEED;  //电脑按键发送的速度  
				  ...
				  break;
		      case RC_SW_MID://遥控操控，遥控发送的速度（RemotData.rc.ch1），RemotData.rc.ch1减去1027乘上REMOT_HORIONTALSPEED就是遥控器控制电机的速度
				  {
					  control.CMControl.horizoltal        = (RemotData.rc.ch0 - 1024) * REMOT_HORIONTALSPEED;//水平向左向右
					  control.CMControl.forwardOrbackward = (RemotData.rc.ch1 - 1024) * REMOT_FORWARDSPEED;
					  control.CMControl.DnfanceAngle = 0;					  

					   if((ControlBits & 0x10) == 0) //云台
					   {
					     control.CMControl.left_right += (RemotData.rc.ch2 -1024) * REMOT_LEFTRIGHT_SPEED;//没有云台，用云台的通道2来操控车子的左右转（把传给云台的数据GimbalControl.left_right -变为传给底盘的数据control.CMControl.left_right +），轮子的数据还是会传到底盘电机的运动数据里计算
							// control.GimbalControl.left_right -= (RemotData.rc.ch2 -1027) * REMOT_LEFTRIGHT_SPEED;
						 // control.GimbalControl.updown     -= ((float)(RemotData.rc.ch3 -1027)) * REMOT_UPDOWN_SPEED; 	
					   }
  ...
  for(;;)
	{
	   ...
	   switch(ControlBits)
	   {
           ...
           case 3: 
               SetFireSpeed(125); //摩擦轮转速
		       SetFireFre(120 * M2006_REDUCTIONRTION);  //拨蛋轮转速
			   break;
           case 2:  		   
			   SetFireSpeed(120); 
		       SetFireFre(0);
...
//		
     	...
     	//这个判断语句是使遥控拨杆回到中心点的时候四个轮子左右转停止（原因：云台跟随的时候，拨杆不到中心点，轮子的左右转不会停，所以在跟随里面手动把左右转的运动函数置0）		
			if(controlvalue.DnfanceAngle ==  0u) //正常跟随
			{
			   // controlvalue.left_right = 0;//((YawMotor.position.encoderInitValue - YawMotor.position.encoder_p.lastValue)*360u /8192u ) * CM_FOLLOW_SPEED;
				//L_Or_R = 0;
				control.CMControl.left_right=0;
			}	 
//			else
...
//			   if(L_Or_R == 0) //防御姿态
//			   {			
//				   controlvalue.left_right =0;// controlvalue.DnfanceAngle * controlvalue.DnfanceSpeed;
//				   
...
{	
	...
	TickType_t PreviousWakeTime;
	const TickType_t TimerIncrement =  pdMS_TO_TICKS(6);
	PreviousWakeTime = xTaskGetTickCount();
    PID_Parms_Init();
    for(;;)
    {
	 ...
	 // YawMotor.speed.setSpeed = 0;//调试速度环时使用
	  ...
	  //output[0]       =0;//关闭Y轴
	   ...
	   for(char i = 0;i<4;i++)
			...
			//用can接收电机目前的速度CM_Motor[i].getspeed，并且和你要发送的速度CM_Motor[i].setSpeed进行PID运算
	      cmspeed[i]   =   (int16_t)pid_calc(&CM_Motor[i].pid,CM_Motor[i].setSpeed,CM_Motor[i].getspeed);
			
			
//			
//			cmspeed[0]   =   (int16_t)pid_calc(&CM_Motor[0].pid,CM_Motor[0].setSpeed,CM_Motor[0].getspeed);
//       cmspeed[1] =0;
//			 cmspeed[2] =0;
//			 cmspeed[3] =0;
	 ...
	 CanSendMess(CAN1,SEND_ID205_207,output); //经过PID计算的值存储在output里面，再通过can发送出去
     CanSendMess(CAN1,SEND_ID201_204,cmspeed); //经过PID计算的值存储在cmspeed里面，再通过can发送出去
...
static void OfflineDetectedTask(void *pvParameters)
{
    ...
    else
	{
	  ...
	  vTaskDelay(10);
		  CanSendMess(CAN1,SEND_ID201_204,out); 
	}
}
