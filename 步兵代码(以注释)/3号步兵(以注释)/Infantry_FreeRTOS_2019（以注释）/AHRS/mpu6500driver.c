...
//这个文件里都是一些MPU6500的初始化（调用配置函数）和读写MPU6500操作的函数
//一般是利用SPI通信协议往MPU的寄存器的某些位进行读写操作
...
//这个数组有14组，每一组里有3个数据，[MPU6500_Write_Reg_Num][0]的数据为寄存器地址 
//                                     [MPU6500_Write_Reg_Num][1]为写入寄存器的数据
//                                    [MPU6500_Write_Reg_Num][2] 错误码   
static const uint8_t write_mpu6500_reg_data_error[MPU6500_Write_Reg_Num][3] =
    {
        ...
        {MPU_SMPLRT_DIV,
         MPU_SMPLRT_DIV_1,//设置分采样率，即每秒采样多少次
         SMPLRT_DIV_ERROR},
...
uint8_t mpu6500_init(void)
{
    ...
    mpu6500_GPIO_init();
    // mpu6500 com init
    mpu6500_com_init();  //初始化SPI

    //check commiunication is normal
    mpu6500_read_single_reg(MPU_WHO_AM_I);//验证MPU的ID
    mpu6500_delay_us(wait_time);
    mpu6500_read_single_reg(MPU_WHO_AM_I);
    mpu6500_delay_us(wait_time);

    mpu6500_write_single_reg(MPU_PWR_MGMT_1, MPU_DEVICE_RESET);//往MPU_PWR_MGMT_1寄存器中写入MPU_DEVICE_RESET，解除休眠状态
    ...
    res = mpu6500_read_single_reg(MPU_WHO_AM_I);//读取当前的MPUID号，确认传感器是否正常
    mpu6500_delay_us(wait_time);
    if (res != DEVICE_ID)//看读到的ID号是否为0x70
    {
        return NO_Sensor;
    }
    ...
    {
			//write_mpu6500_reg_data_error［］［］是一个数组，数组里面存放的是MPU配置的寄存器地址和配置数据
			//[MPU6500_Write_Reg_Num][0]为要写入的寄存器地址， [MPU6500_Write_Reg_Num][1] 为写入的数据
			//是为了达到批量设置的目的
			//在这里边设置了测量范围等待
...
//ACCEL_SEN和GYRO_SEN这个就是根据分辨率计算出来的１LSB＝0.00059814453125fm/s＾２或0.0005326322180158476492076f度
//ｘLSB前面的ｘ就是ADC转换出来数据量，只要ｘ乘上１LSB＝后面那一串就可以算出角速度和角加速度

//在这个函数里会调用读取MPU原始值的函数
//陀螺仪真正的原始数据temp_imu_data，乘上ACCEL_SEN化为国际单位，在赋值给mpu6500_real_data
void mpu6500_read_over(uint8_t *status_buf, mpu6500_real_data_t *mpu6500_real_data)
{
    ...
    if ((*status_buf) & MPU_RAW_RDY_INT)
    {
        ...
        mpu6500_real_data->accel[0] = temp_imu_data * ACCEL_SEN;  //陀螺仪真正的原始数据temp_imu_data，乘上ACCEL_SEN或GYRO_SEN化为国际单位
...
//读取角速度就是用SPI通信协议来读取MPU_GYRO_XOUT寄存器的值
void mpu6500_read_gyro(fp32 gyro[3])//读取角速度的原始值赋给temp_imu_data
{
    uint8_t buf[6];
    int16_t temp_imu_data = 0;
...
//读取角速度就是用SPI通信协议来读取MPU_ACCEL_XOUT寄存器的值
void mpu6500_read_accel(fp32 accel[3])//读取角加速的原始值赋给
{
    uint8_t buf[6];
    int16_t temp_imu_data = 0;
...
void mpu6500_read_temp(fp32 *temperature)//读取温度
{
    uint8_t buf[2];
    int16_t temp_imu_data = 0;
