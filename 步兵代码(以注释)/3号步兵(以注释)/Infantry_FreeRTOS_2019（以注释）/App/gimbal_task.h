...
//电机码盘值最大以及中值
#define Half_ecd_range 4096//中值
#define ecd_range 8191  //最大值
...
#define INIT_YAW_SET 0.0f  //云台初始化的角度
#define INIT_PITCH_SET 0.0f  //云台初始化的角度
...
typedef struct
{
    ...
    uint16_t offset_ecd; //云台电机处于Y轴或P轴中间时编码器的值
	
	//最大相对角度和最小相对角度的赋值是在云台校准时，云台自己摆动到极限位置，并且记录下最大相对角度和最小相对角和云台中值
    fp32 max_relative_angle; //rad
    fp32 min_relative_angle; //rad

    fp32 relative_angle;     //rad
	//说明：云台的相对角度relative_angle就是云台此时的状态距离云台在中间时的角度（即就是云台和底盘之间的偏差角度）
	//说明：relative_angle来源：在GIMBAL_Feedback_Update函数中把读出来的电机编码值和云台值进行计算得到
	
    fp32 relative_angle_set; //rad
	//说明：进行PID计算时relative_angle_set为设置值，relative_angle当作反馈值
	
	
	
	//gimbal_yaw_motor.absolute_angle就是Y轴的姿态角、gimbal_pitch_motor.absolute_angle就是P轴的姿态角
    fp32 absolute_angle;     //rad
		//说明：这个就是云台的姿态角 来源：在GIMBAL_Feedback_Update函数中把从MPU读出来的云台姿态角gimbal_INT_angle_point得到
...
typedef struct
{
    ...
    uint8_t step;  //模式控制里gimbal_cali.step 
} Gimbal_Cali_t;

typedef struct
{
    const RC_ctrl_t *gimbal_rc_ctrl; //遥控传来的数据
    const fp32 *gimbal_INT_angle_point;
    const fp32 *gimbal_INT_gyro_point;
    Gimbal_Motor_t gimbal_yaw_motor;  //这个的变量类型为Gimbal_Motor_t，也就是说gimbal_yaw_motor可以调用Gimbal_Motor_t结构体里的成员
    Gimbal_Motor_t gimbal_pitch_motor;//这个的变量类型为Gimbal_Motor_t，也就是说gimbal_pitch_motor可以调用Gimbal_Motor_t结构体里的成员
    Gimbal_Cali_t gimbal_cali;   //模式控制里的gimbal_mode_set->gimbal_cali
	AUTODATA *autodata;
	char auto_aim_flag;
} Gimbal_Control_t;
