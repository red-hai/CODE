...
void calibrate_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        {
            //校准命令为1 表示需要校准
            if (cali_sensor[i].cali_cmd)//云台校准时cali_sensor[1].cali_cmd=1，陀螺仪校准时cali_sensor[2].cali_cmd=1
            {
                if (cali_sensor[i].cali_hook != NULL)
                {
                    //调用校准函数
                    if (cali_sensor[i].cali_hook(cali_sensor_buf[i], CALI_FUNC_CMD_ON))  //云台校准调用cali_gimbal_hook（）函数，陀螺仪用另外的函数
                    {
                        //校准完成
...
static void RC_cmd_to_calibrate(void)
{
    ...
    else if (rc_action_falg == 2 && rc_cmd_time > RC_CMD_LONG_TIME)
    {
        ...
        rc_action_falg = 0;
        rc_cmd_time = 0;
        cali_sensor[CALI_GIMBAL].cali_cmd = 1;  //上外八使能云台校准，把cali_sensor[CALI_GIMBAL].cali_cmd置1，即cali_sensor[1].cali_cmd置1
    }
    else if (rc_action_falg == 3 && rc_cmd_time > RC_CMD_LONG_TIME)
    {
        ...
        rc_action_falg = 0;
        rc_cmd_time = 0;
        cali_sensor[CALI_GYRO].cali_cmd = 1;////上内八使能云台校准，把cali_sensor[CALI_GYRO].cali_cmd置1，即cali_sensor[12.cali_cmd置1
        //更新MPU6500需要控制的温度
...
static bool_t cali_gimbal_hook(uint32_t *cali, bool_t cmd)//云台校准时传进来的cmd为CALI_FUNC_CMD_ON，他的宏为1
{
    ...
    else if (cmd == CALI_FUNC_CMD_ON)  //云台校准执行这里
    {
			//中值数据生成函数，在下边的函数里把gimbal_control.gimbal_cali.step 的值赋为GIMBAL_CALI_START_STEP;
