...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    else if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[MODE_CHANNEL]))
    {
		...
		//车子运动时用的是chassis_behaviour_mode = CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW;
    ...
    //云台进入某些状态的时候，底盘保持不动
		//云台无力和云台校准和云台初始化时，底盘保存不动，这里的由云台的gimbal_behaviour来决定
    ...
    else if (chassis_behaviour_mode == CHASSIS_NO_MOVE)
    {
		...
		//车子运动时是chassis_move_mode->chassis_mode = CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW
...
//车子运动时是chassis_move_mode->chassis_mode = CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW
    ...
    if (chassis_behaviour_mode == CHASSIS_ZERO_FORCE)  //底盘无力
    {
        chassis_zero_force_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);//前三个参数都置0，底盘就不动
    }
    else if (chassis_behaviour_mode == CHASSIS_NO_MOVE)  //底盘保持不动
    {
        chassis_no_move_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);//前三个参数都置0，底盘就不动
    }
		
		//车子运动（底盘跟随云台）用的模式
		//小蛮腰状态是写在底盘跟随云台模式下（因为摇摆退出后，底盘还要回到和云台一样的状态）
    else if (chassis_behaviour_mode == CHASSIS_INFANTRY_FOLLOW_GIMBAL_YAW)//步兵底盘跟随云台模式
    {
        chassis_infantry_follow_gimbal_yaw_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
    }
		
		
    else if (chassis_behaviour_mode == CHASSIS_ENGINEER_FOLLOW_CHASSIS_YAW)//工程底盘角度控制底盘，由于底盘未有陀螺仪，故而角度是减去云台角度而得到，如果有底盘陀螺仪请更新底盘的yaw，pitch，roll角度
    {
        chassis_engineer_follow_chassis_yaw_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
    }
		
		//车子运动（底盘和云台单独控制）用的模式
    else if (chassis_behaviour_mode == CHASSIS_NO_FOLLOW_YAW)////底盘不跟随角度，角度是开环的，但前后左右是有速度环
    {
        chassis_no_follow_yaw_control(vx_set, vy_set, angle_set, chassis_move_rc_to_vector);
    }
		
    else if (chassis_behaviour_mode == CHASSIS_OPEN)//遥控器的值乘以比例直接发送到can总线上
    {
...
static void chassis_infantry_follow_gimbal_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    if (vx_set == NULL || vy_set == NULL || angle_set == NULL || chassis_move_rc_to_vector == NULL)
    {
		...
		////遥控器和键盘的数据处理成底盘的前进vx速度，vy速度
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
		//在里面把遥控和键盘的数据处理成了vy_set和vx_set车子前进和左右的速度
		//angle_set的赋值不是从这里面得来，而从下面的代码得来
...
//使能摇摆标志位，swing_flag为摇摆标志位
    ...
    {
        max_angle = SWING_MOVE_ANGLE;//这是从运动状态进入摇摆状态的摇摆角度的比例
    }
    else  //
    {
        max_angle = SWING_NO_MOVE_ANGLE;//这是从静止状态进入摇摆状态的摇摆角度的比例
    }
    //sin函数生成控制角度
    if (swing_flag)//如果swing_flag摇摆标志位为1，就生成摇摆角度
    {
    ...
    {
			//如果swing_flag摇摆标志位为０，swing_angle就不代表摇摆角度，并且把swing_angle置０
        swing_angle = 0.0f;
    }
    //sin函数不超过2pi
    if (swing_time > 2 * PI)
    ...
    *angle_set = swing_angle;//把生成摇摆角度赋给angle_set
}
...
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
    *wz_set = -CHASSIS_WZ_RC_SEN * chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_WZ_CHANNEL];//这里把云台控制底盘旋转变为遥控的通道2通过遥控器控制旋转
}
