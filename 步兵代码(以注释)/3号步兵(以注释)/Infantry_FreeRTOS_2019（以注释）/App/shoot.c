...
void shoot_init(void)
{
    ...
    //电机指针
  //  for(char i = 0;i<3;i++)//如有多个拨弹轮电机，才用这一句
	// {
															//获取拨弹轮电机的数据
    ...
    Shoot_Feedback_Update();
	 ...
	 //斜波初始化函数（主要初始化摩擦轮电机，摩擦轮启动要斜波，拨弹轮启动不用斜波）
...
int16_t *shoot_control_loop(void)
{
   static  int16_t shoot_CAN_Set_Current[5]; //返回的can值
    ...
    //发射状态控制，先把摩擦轮开启即SHOOT_BULLET，然后再开启拨弹轮
...
//射击先进入SHOOT_READY就绪模式，才能进入射击模式SHOOT_BULLET
static void Shoot_Set_Mode(void)
{
    static int8_t last_s = RC_SW_UP;//刚开始是设置为拨杆的上一次为上档，这个设置的上档只用一次

	
	//这是遥控开启摩擦轮，左边拨杆上拨一次进入就绪状态SHOOT_READY，第二次上拨就进入关闭状态SHOOT_STOP
    //上拨判断， 一次就绪，再次关闭
    ...
    else if ((switch_is_up(shoot_rc->rc.s[Shoot_RC_Channel]) && !switch_is_up(last_s) && shoot_mode != SHOOT_STOP) || (shoot_rc->key.v & SHOOT_OFF_KEYBOARD))
    {
		...
		//这是键盘开启摩擦轮，左边拨杆处于中档按下z键就进入就绪状态SHOOT_READY
    //处于中档， 可以使用键盘就绪摩擦轮
    ...
    if (gimbal_cmd_to_shoot_stop())
    {
		...
		//这里是把上面的就绪状态转换为射击状态，前提是shoot_mode已经为就绪模式SHOOT_READY
    ...
    last_s = shoot_rc->rc.s[Shoot_RC_Channel];//记录上一次拨杆的位置
}
/**
  * @brief          射击数据更新
  * @author         RM
