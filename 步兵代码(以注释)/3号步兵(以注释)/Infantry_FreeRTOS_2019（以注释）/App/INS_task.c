...
//如果不使用SPI的方法，则使用普通SPI通信的方法
        ...
        //将读取到的mpu6500原始数据处理成国际单位的数据，mpu6500_real_data中为化作国际单位的原始数据
...
//将读取到的ist8310原始数据处理成国际单位的数据
        ...
        //减去零漂以及旋转坐标系
				//把MPU读出来的化作国际单位的原始数据mpu6500_real_data和读取到的ist8310化作国际单位的原始数据ist8310_real_data传入这里边，得到INS_gyro, INS_accel, INS_mag
        IMU_Cali_Slove(INS_gyro, INS_accel, INS_mag, &mpu6500_real_data, &ist8310_real_data);
				 //得出来的INS_gyro, INS_accel, INS_mag还是原始数据，自不过是比较精确
        //得出来的INS_gyro, INS_accel, INS_mag经下面的出来就会得出姿态角欧拉角INS_Angle
        


         //这下面就是对MPU和ist的数据进行姿态融合处理
        //加速度计低通滤波
        ...
        if( mpu6500_real_data.status & 1 << MPU_DATA_READY_BIT)
        {
            ...
            if (updata_count == 0)
            {
                ...
                //初始化四元数，INS_quat为四元数，INS_Angle为欧拉角
                AHRS_init(INS_quat, INS_accel, INS_mag);
                get_angle(INS_quat, INS_Angle, INS_Angle + 1, INS_Angle + 2);
							//这两个函数就是得出最终的欧拉角INS_Angle（包含Roll、P轴、Y轴轴）
...
//这里把原始数据经过四元数和DMP处理过得到的最终的姿态角即欧拉角INS_Angle传出去
...
//这里把MPU得出来的原始数据（原始数据已经转换为角度的单位）INS_gyro传出去
const fp32 *get_MPU6500_Gyro_Data_Point(void)
{
    return INS_gyro;
}

////这里把MPU得出来的原始数据（原始数据已经转换为角加速度的单位）INS_accel传出去
