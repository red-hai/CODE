...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    //遥控器设置行为模式   rc.s[MODE_CHANNEL] 取右边的小拨杆  MODE_CHANNEL为0
    if (switch_is_mid(chassis_move_mode->chassis_RC->rc.s[MODE_CHANNEL]))  //中档，rc.s[MODE_CHANNEL]是否为3
    {
        chassis_behaviour_mode = CHASSIS_NO_FOLLOW_YAW;//底盘不跟随云台模式
    }
    else if (switch_is_down(chassis_move_mode->chassis_RC->rc.s[MODE_CHANNEL]))//下档，rc.s[MODE_CHANNEL]是否为2
    {
        chassis_behaviour_mode = CHASSIS_NO_MOVE;  //底盘无动作模式
    }
    else if (switch_is_up(chassis_move_mode->chassis_RC->rc.s[MODE_CHANNEL]))  //上档，rc.s[MODE_CHANNEL]是否为1
    {
...
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
		...
		//底盘云台不跟随模式下，通道2的值也就是y轴的值转换为底盘的左右旋转
