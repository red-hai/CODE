...
float    freOfSmallBullet =  -2.5,    freOfBigBullet = 8.0f,speedOfBigBullet = 15.0f;
...
//    }
    ...
    {
		//小弹丸拨弹轮控制
		if(trigger_motor[2].press_l == 1 || switch_is_down(shoot_rc->rc.s[Shoot_RC_Channel]))
		{
			if(trigger_motor[2].move_flag == 0)
			{
				trigger_motor[2].set_angle = rad_format(trigger_motor[2].set_angle + PI_Eigth);
				trigger_motor[2].cmd_time  = xTaskGetTickCount();
				trigger_motor[2].move_flag = 2;		
			} 
			
			if (rad_format(trigger_motor[2].set_angle - trigger_motor[2].angle) > 0.05f)
			{
				//角度达到判断
				trigger_motor[2].speed_set = freOfSmallBullet;
				trigger_motor[2].run_time  = xTaskGetTickCount();
				//堵转判断
				if (trigger_motor[2].run_time - trigger_motor[2].cmd_time > 400 && trigger_motor[2].run_time - trigger_motor[1].cmd_time < 400 + 700 && fabs(trigger_motor[1].speed) < (0.5f))
				{
					trigger_motor[2].speed_set = - 8.0;
				}
				else if (trigger_motor[2].run_time - trigger_motor[2].cmd_time > 400 + 700)
				{
					trigger_motor[2].cmd_time = xTaskGetTickCount();
				}
			}
		...
		{
			trigger_motor[2].set_angle = trigger_motor[1].angle;
			trigger_motor[2].move_flag = 0;
...
static void LevelToStopBulet()
{
	...
	if(trigger_motor[0].referee_data->power_heat_data.shooter_heat0 > trigger_motor[0].referee_data->game_robot_state.shooter_heat0_cooling_limit -30.0f)
	{
		trigger_motor[1].speed_set = 0;
	}

	if(trigger_motor[0].referee_data->power_heat_data.shooter_heat1 > trigger_motor[0].referee_data->game_robot_state.shooter_heat1_cooling_limit-100.0f)
	{
		shoot_mode = SHOOT_READY;
		trigger_motor[2].speed_set = 0;
	}
