...
void chassis_task(void *pvParameters)
{
    ...
    //底盘初始化　ok 
    chassis_init(&chassis_move);//chassis_move为遥控给他的状态值
    //判断底盘电机是否都在线
...
//		}
        ...
        if (!(toe_is_error(ChassisMotor1TOE) || toe_is_error(ChassisMotor2TOE) || toe_is_error(ChassisMotor3TOE) || toe_is_error(ChassisMotor4TOE)))
        {
            ...
            else
            {
				for(char i = 0;i<4;i++)
				{
					chassis_set_current[i] =0;// chassis_move.motor_chassis[i].give_current;
				}
            }			
...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    if (chassis_move_init == NULL)
    {
		...
		//把kp ki kd 三个用来计算的值存在相应数组里
		
    //底盘速度环pid值，fp32是浮点型，然后传入PID的初始化函数PID_Init里面去，把值赋给相应的kp ki kd
    const static fp32 motor_speed_pid[3] = {M3505_MOTOR_SPEED_PID_KP, M3505_MOTOR_SPEED_PID_KI, M3505_MOTOR_SPEED_PID_KD};
		
		
    //底盘旋转环pid值，传入PID的初始化函数PID_Init里面去，把值赋给相应的kp ki kd
    ...
    //底盘开机状态为停止
    chassis_move_init->chassis_mode = CHASSIS_VECTOR_RAW;   //状态3
    //获取遥控器指针
