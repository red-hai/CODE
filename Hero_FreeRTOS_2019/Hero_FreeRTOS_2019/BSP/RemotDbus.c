...
void RemotDbus_Init()
{
	...
	DMA_InitStructure.DMA_BufferSize = 36;	//接收原始数据，为18个字节，给了36个字节长度，防止DMA传输越界															
...
//遥控的值通过函数指针的形式来传递给其他任务
const RC_ctrl_t *get_remote_control_point(void)
{
    return &rc_ctrl;  //rc_ctrl为结构体指针，存储着遥控数据经过解析后的数据
}
