...
//    //使能摇摆标志位
		...
		//360旋转
		 static fp32 revolve_angle=0.0f;
		static fp32  revolve_add=0.0f;
 

    //计算遥控器的原始输入信号

		//小陀螺：要做的就是把旋转角度算出来
		if (chassis_move_rc_to_vector->chassis_RC->key.v & REVOLVE_START_KEY)
		{
		//第一种方法	
		revolve_add=0.3f;
...
//			
			...
			revolve_angle=revolve_add;
		}
		else if(chassis_move_rc_to_vector->chassis_RC->key.v & REVOLVE_STOP_KEY)
		{
		revolve_add=0.0f;
			revolve_angle=revolve_add;
		revolve_flag=0;
