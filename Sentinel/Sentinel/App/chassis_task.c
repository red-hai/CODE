...
static void chassis_init(chassis_move_t *chassis_move_init)
{
	...
	chassis_move_init->mainfocus_data = GetAutoDataPoint();
    //初始化PID 运动
    for (i = 0; i < 4; i++)
    {
...
{
	static int16_t cnt = 0,moveflag = 0 ;
	static char left_ritht = 0;
	...
	if((chassis_move_rc_to_vector->mainfocus_data->PitchAxiaAngle == 0.0f && chassis_move_rc_to_vector->mainfocus_data->YawAxiaAngle == 0.0f) )
	{
		 cnt++;
		 if(cnt >= 1000 )
		 {
	...
	{	
		chassis_move_rc_to_vector->auto_move_speed = 0;
		moveflag = 0;
	    cnt = 0;
	}
	
	if(moveflag == 1 && toe_is_error(DBUSTOE))
	{
	   if(LIMIT_LEFT == 0 && LIMIT_RIGTT == 1)
	   {
		  left_ritht = 0;
	   }
	   else if(LIMIT_RIGTT == 0 &&  LIMIT_LEFT == 1)
	   {
		  left_ritht = 1; 
	      
	   }
	   else if(LIMIT_RIGTT == 0 &&  LIMIT_LEFT == 0)
	   {
		    left_ritht = 2;    
	   }
	   ...
	   if(left_ritht == 0)
	   {
	       chassis_move_rc_to_vector->auto_move_speed = -3.0f; 
	   }
	   else  if (left_ritht == 1)
	   { 
	      chassis_move_rc_to_vector->auto_move_speed = 3.0f; 
	   }
	   else 
    ...
    vx_set_channel = vx_channel * CHASSIS_VX_RC_SEN + chassis_move_rc_to_vector->auto_move_speed;
...
static void CalcChassisMaxOutput()
{
	...
	error_out = (200.0f - GetPowerBuffer()) * 0.040;	
