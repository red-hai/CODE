...
void GIMBAL_task(void *pvParameters)
{
	...
	while(1)
	{
        ...
        if (!(toe_is_error(YawGimbalMotorTOE) && toe_is_error(PitchGimbalMotorTOE) && toe_is_error(TriggerMotorTOE)))
        {
			...
			gimbal_set_current[2] = shoot_out[0];
...
static void GIMBAL_Control_loop(Gimbal_Control_t *gimbal_control_loop)
{
    ...
    else if (gimbal_control_loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO)
    {
		...
		if(((gimbal_control_loop->autodata->PitchAxiaAngle != 0) || (gimbal_control_loop->autodata->YawAxiaAngle != 0)) &&  last_aim_flag == 0  && auto_flag == 0 )
		{
		   cnt = 0;
		   auto_flag = 1; 
		}
		else if(((gimbal_control_loop->autodata->PitchAxiaAngle == 0) && (gimbal_control_loop->autodata->YawAxiaAngle == 0))&& last_aim_flag == 1 && auto_flag == 1)
		{
	        cnt++;
            if(cnt>= 500)
			{
		...
		if(auto_flag == 1)
		{	
			static float yaw_add =  0;// 1.0f/180.0f * 3.14f;	

			if(getEenmyColor() == 0)
			{
				yaw_add =  1.0f/180.0f * 3.14f;
			}
			else
			{
				yaw_add =  1.0f/180.0f * 3.14f;
    ...
    else if (gimbal_control_loop->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
    {
		...
		{	
			...
			static float pitch_add =  0;//2.0f/180.0f * 3.14f;
			if(getEenmyColor() == 0)
			{
				pitch_add =  -1.0f/180.0f * 3.14f;
			}
			else
			{
				pitch_add =  2.0f/180.0f * 3.14f;
			}
...
//absolute_angle_set用来记录上一次转到的角度，其实用来记录用用遥控控制时云台转到的角度，就可以使云台在自动转时可以接着上次用遥控把云台控到的那个位置开始转
		//                                           还记录自动转时云台停止的角度，就可以使云台在用遥控控制转时可以接着上次自动转把云台转到到的那个位置开始转
